...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (csvColumn >= cols.length || csvColumn < 0) {
            return null;
        }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        try {
            while ((newLine = br.readLine()) != null) {
                String tweet = extractColumn(newLine, tweetColumn);
                if (tweet != null) {
                    tweets.add(tweet);
                }
            }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        String sanitizeTweet = removeURLs(tweet);
        List<List<String>> sentences = new LinkedList<>();

        for (String sentence : tweetSplit(sanitizeTweet)) {
            List<String> cleanedWords = parseAndCleanSentence(sentence);

            if (!cleanedWords.isEmpty()) {
                sentences.add(cleanedWords);
            }
        }
        return sentences;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(BufferedReader br, int tweetColumn) {
        ...
        List<List<String>> trainingDataExamples = new LinkedList<>();

        for (String tweet : tweets) {
            List<List<String>> sentences = parseAndCleanTweet(tweet);
            trainingDataExamples.addAll(sentences);
        }
        return trainingDataExamples;
    }
