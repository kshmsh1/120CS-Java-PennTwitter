 ...
 * <p>
 * The Markov Chain that results from this training data maps each observed
...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ProbabilityDistribution<String> second1;
        ...
        if (!chain.containsKey(first)) {
            second1 = new ProbabilityDistribution<String>();
        } else {
            second1 = chain.get(first);
        }
        second1.record(second);
        chain.put(first, second1);
    }
    ...
    public void train(Iterator<String> sentence) {
        if (sentence == null) {
            ...
            String previous = sentence.next();

            if (!previous.isEmpty()) {
                startWords.record(previous);

                while (sentence.hasNext()) {
                    String next1 = sentence.next();
                    addBigram(previous, next1);
                    previous = next1;
                }
                addBigram(previous, END_TOKEN);
            }
        }
    }
    ...
    public boolean hasNext() {
        return !END_TOKEN.equals(current);
    }
    ...
    public String next() {
        ...
        if (chain.containsKey(current)) {
            next1 = chain.get(current).pick(ng);
        } else {
            next1 = END_TOKEN;
        }
